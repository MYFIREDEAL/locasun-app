...
const ChatInterface = ({ prospectId, projectType, currentStepIndex, activeAdminUser, initialChannel }) => {
  ...
  const sendNextAction = useCallback(async (completedActionId = null) => {
    ...
    // Le trigger DB gère les subSteps + complétion d'étape.
    // L'admin clique sur le robot pour lancer chaque action.
    // sendNextAction n'est PLUS appelé automatiquement pour V2.
    // ═══════════════════════════════════════════════════════════════════
    if (completedActionId?.startsWith('v2-')) {
      logger.info('🔄 [V2] sendNextAction: trigger DB gère subSteps + complétion. Admin relancera le robot pour action suivante.', { completedActionId });
      return;
    }
