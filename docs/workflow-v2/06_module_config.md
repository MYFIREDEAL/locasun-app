...
| `src/components/admin/workflow-v2/ModuleConfigPanel.jsx` | UI d'édition (drawer) |
| `src/components/admin/workflow-v2/ModuleConfigTab.jsx` | UI d'édition (onglet inline) |

---

## Onglet "Workflow V2" dans ModulePanel

### Position

L'onglet "Workflow V2" est accessible dans la barre de navigation du module, entre l'onglet "Contact" et le futur onglet "Charly" :

```
┌─────────────────────────────────────────────────────────────────┐
│  📋 Inscription                                   ✅ Terminé   │
├─────────────────────────────────────────────────────────────────┤
│  [ 👤 Contact ]  [ ⚙️ Workflow V2 ]                             │
│  ────────────────────────────────────────────────────────────   │
├─────────────────────────────────────────────────────────────────┤
│  ✨ Configuration IA                                            │
│     Module : Inscription                          🛡️ Temp.      │
│                                                                 │
│  ┌ Info ─────────────────────────────────────────────────────┐  │
│  │ Les modifications sont appliquées en mémoire...           │  │
│  └───────────────────────────────────────────────────────────┘  │
│                                                                 │
│  ✨ Objectif du module                                          │
│  ┌───────────────────────────────────────────────────────────┐  │
...
│  ⚡ Labels des boutons                                          │
│  ┌─────────────────────┐ ┌─────────────────────┐                │
│  │ PROCEED: Valider    │ │ NEED_DATA: Infos    │                │
│  └─────────────────────┘ └─────────────────────┘                │
│                                                                 │
│  📖 Actions autorisées              [Lecture seule]             │
│  [💬 Répondre] [✅ Checklist] [📄 Documents]                    │
│                                                                 │
│  📚 Base d'info liée                [Lecture seule]             │
│  [ inscription ]                                                │
│                                                                 │
│  ────────────────────────────────────────────────────────────   │
│                              [ ↺ Réinitialiser ] [ 💾 Sauv. ]   │
...
|-------|------|------------|-------------|
| **Objectif** | textarea | ✅ Oui | Objectif principal du module |
| **Instructions IA** | textarea (large) | ✅ Oui | Instructions détaillées pour l'IA |
| **Label PROCEED** | input | ✅ Oui | Label du bouton de validation |
| **Label NEED_DATA** | input | ✅ Oui | Label du bouton de demande d'infos |
| **Actions autorisées** | tags | ❌ Non (READ_ONLY) | Actions que l'IA peut effectuer |
| **Base d'info** | text | ❌ Non (READ_ONLY) | Clé vers la base de connaissance |

### Règle de persistance

**⚠️ IMPORTANT : Pas de persistance tant que PROMPT 9 non validé.**

Les modifications sont :
1. Appliquées immédiatement en mémoire (state local)
2. Sauvegardées via `updateModuleAIConfig()` (in-memory)
3. **Perdues au rechargement de la page**

Pour activer la persistance Supabase, voir `05_supabase_migrations.md`.
