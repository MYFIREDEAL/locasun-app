...
| 4 mars 2026 | **Onglet Développeur — Add Project** : Section complète dans IntegrationsPage : contrat JSON (action + prospect_id + type_projet), réponse succès 201, exemples curl + fetch, table codes erreurs spécifiques (MISSING_FIELDS, INVALID_PROSPECT, DUPLICATE_PROJECT). |
| 4 mars 2026 | **Module Make "Add Project" (doc)** : `MAKE_ADD_PROJECT_MODULE.md` — guide complet pour configurer le 2ème module sur Make Developer Platform (Parameters, Communication, Output, tests). Section Module 2 ajoutée dans onglet Make App EVATIME. |
| 4 mars 2026 | **Diagnostic Rosca Finance** : Les types `piscine-copie-*` n'existent plus — 4 types propres : `baby`, `fenetre`, `piscine`, `solaire`. Script renommage supprimé (inutile). |
| 4 mars 2026 | **Webhook externe par org** : Colonne `external_webhook_url` ajoutée à `integration_keys`. `webhook-v1` appelle automatiquement cette URL (POST, fire-and-forget) après création prospect — payload : `event`, `prospect_id`, `owner_id`, `nom`, `email`, `telephone`, `type_projet`, `tags`, `organization_id`, `magic_link_sent`, `created_at`. |
